...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        /// <summary>
        /// フィルターに一致するランダムな部屋を結合します。コールバックされます。OnJoinedRoomまたはOnJoinRandomFailed。
        /// </summary>
        /// <remarks>
        /// ランダムマッチングに使用します。任意の部屋、または opJoinRandomRoomParams で定義された特定のプロパティを持つ部屋に参加することができます。
        ///
        /// この操作は、適合する部屋または利用可能な部屋がない場合（すべて満室、閉鎖、他のロビーにある、または表示されていない）、失敗します。
        /// また、見つかった部屋に実際に入室する際に失敗することもあります。部屋はいつでも閉鎖、満室、空室となる可能性があります。
        ///
        /// このメソッドは、クライアントがマスターサーバーに接続している間のみ呼び出すことができるので、以下のようにします。
        /// コールバック OnConnectedToMaster を実装する。
        /// 戻り値を確認し、サーバー上で操作が呼び出されることを確認します。
        /// 注意：このメソッドがfalseを返した場合、コールバックは行われません。
        ///
        /// PUNのお見合いについてはこちら
